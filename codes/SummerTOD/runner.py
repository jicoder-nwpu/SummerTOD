...
class MultiWOZRunner(BaseRunner):
    def __init__(self, cfg):
        ...
        reader = MultiWOZReader(cfg.backbone, cfg.version, self.logger, cfg.summary_context_size)
        ...
        for dial_batch in tqdm(pred_batches, total=len(pred_batches), desc="Prediction"):
            ...
            for turn_batch in self.iterator.transpose_batch(dial_batch):
                ...
                for turn in turn_batch:
                    ...
                    # dbpn.append(turn['bspn'] + turn["dbpn"])
                    dbpn.append([])
    ...
    def response(self, user, history, last_summary):
        self.model.eval()

        user = self.reader.encode_text(user,
        ...
        for sent in history:
            dial_history.append(self.reader.encode_text(sent))
        ...
        summary_context, last_response = self.iterator.flatten_dial_history(
            dial_history, len(user), self.cfg.summary_context_size)
        
        summary_history_encoder_input_ids = summary_context + last_response + user + [self.reader.eos_token_id]
        
        reponse_context, last_response = self.iterator.flatten_dial_history(
            dial_history, len(user), self.cfg.context_size)

        response_history_encoder_input_ids = reponse_context + last_response + user + [self.reader.eos_token_id]
        ...
        if last_summary is None:
            ...
            last_summary_ids = last_summary
        
        updated_history_ids = last_response + user + [self.reader.eos_token_id]
        ...
        dbpn = [[self.reader.pad_token_id] + self.reader.encode_text("<bos_db> [db_3] <eos_db>")]
        ...
        # dial_history.append(user + decoded_resp_outputs['resp_gen'])

        return self.reader.tokenizer.decode(decoded_resp_outputs[0]['resp_gen'][1:-1]), batch_last_summarys[0], self.reader.tokenizer.decode(batch_last_summarys[0][1:-1])
