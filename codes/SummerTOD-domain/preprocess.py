...
class Preprocessor(object):
    def __init__(self, version, sum_window):
        self.nlp = spacy.load("en_core_web_sm")

        self.data_dir = os.path.join("data/MultiWOZ_{}".format(version))
    
        self.sum_prefix = 'this is a summary </s> '
            
        self.sum_window = sum_window

        self.save_dir = os.path.join(self.data_dir, "cross-ws" + str(sum_window) + "-processed")
        ...
        self.dev_list = load_text(os.path.join(self.data_dir, "valListFileMUL.json"))
        self.test_list = load_text(os.path.join(self.data_dir, "testListFileMUL.json"))
        self.train_list = load_text(os.path.join(self.data_dir, "trainListFileSingle.json"))
    ...
    def get_db_values(self):
        if os.path.exists(os.path.join(self.data_dir, 'db')):
            return
        processed = {}
        bspn_word = []
    ...
    def preprocess(self):
        ...
        with open('./all_slot_values.json', 'r') as f:
        ...
        for fn, raw_dial in tqdm(list(self.data.items())):
            ...
            if fn in ['pmul4707.json', 'pmul2245.json', 'pmul4776.json', 'pmul3872.json', 'pmul4859.json']:
                continue
            if fn not in self.train_list and fn not in self.test_list and fn not in self.dev_list:
                continue
            count += 1
            ...
            for turn_num, dial_turn in enumerate(raw_dial['log']):
                ...
                if not dial_state:   # user
                    ...
                    #窗口外对话状态
                    if turn_num > (self.sum_window + 1) * 2 and raw_dial['log'][turn_num - (self.sum_window + 1) * 2]['metadata']:
                        _, belief_value_summary = self.get_summary_bstate(raw_dial['log'][turn_num - (self.sum_window + 1) * 2]['metadata'])
                    ...
                    if single_turn['summary'] == "":
                        "the user has no special needs ."
                        single_turn['summary_delex'] = "the user has no special needs ."
                    else:
                        single_turn['summary'] = single_turn['summary']
                        single_turn['summary_delex'] = single_turn['summary_delex']
                        # single_turn['summary'] = self.sum_prefix + single_turn['summary']
                        # single_turn['summary_delex'] = self.sum_prefix + single_turn['summary_delex']
                    if last_summary == "":
                        single_turn['last_summary'] = "there is no summary ."
                        single_turn['last_summary_delex'] = "there is no summary ."
                    else:
                        single_turn['last_summary'] = last_summary
                        single_turn['last_summary_delex'] = last_summary_delex
                    
                    last_summary = single_turn['summary']
                    last_summary_delex = single_turn['summary_delex']
            ...
            if fn in self.test_list:
                test_data[fn] = dial
            elif fn in self.dev_list:
                dev_data[fn] = dial
...
if __name__ == "__main__":
    ...
    parser.add_argument("-version", type=str, default="2.1", choices=["2.0", "2.1", "2.2"])
    parser.add_argument("-sum_window", type=int, default=100, choices=[2, 3, 4, 5, 6, 7, 100])

    args = parser.parse_args()

    preprocessor = Preprocessor(args.version, args.sum_window)
